...
async def home(client: Client, query: CallbackQuery):
    buttons = [[InlineKeyboardButton("•ᴀʙᴏᴜᴛ•", callback_data = "about"), InlineKeyboardButton("•ᴄʜᴀɴɴᴇʟs•", callback_data = "channels")]]
...
async def premium_select(client, query):
    ...
    text = (
        "<blockquote>✦ Select Your Plan</blockquote>\n"
        "Choose your premium duration below!!!"
    )

    buttons = InlineKeyboardMarkup([
        [InlineKeyboardButton("• Life Time (Limited) •", callback_data="plan_life")],
        [
            InlineKeyboardButton("• One Month •", callback_data="plan_1m"),
            InlineKeyboardButton("Three Months •", callback_data="plan_3m")
        ],
        [
            InlineKeyboardButton("• Six Months •", callback_data="plan_6m"),
            InlineKeyboardButton("Twelve Months •", callback_data="plan_12m")
        ],
...
async def payment_qr(client, query):
    ...
    text = (
        "<blockquote>✦ <b>𝗣𝗥𝗘𝗠𝗜𝗨𝗠 𝗣𝗔𝗬𝗠𝗘𝗡𝗧</b></blockquote>\n"
        f"▪️ Amount: <code>{amount}</code>\n"
        f"▪️ Validity: <code>{validity}</code>\n\n"
